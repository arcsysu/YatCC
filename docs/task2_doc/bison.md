...
同学们需要完成下面两个部分的内容：\
1、由于 task1 的标准答案是复活部分的代码输入，因此同学们需要**补充 lex.cpp 文件**，将 clang 标准输出与语法分析的输入进行匹配；\
2、类型检查和 ASG 生成 json 文件的部分，已经进行了基本的实现。同学们只要认真阅读 asg.hpp 文件，了解每个非终结符对应类型的结构和操作，结合上述重点参考的文法**完成`par.y`文件中的文法撰写和语义动作的补充**。\

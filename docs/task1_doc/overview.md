...
同学们可能会想，实现这样的一个词法分析器的工程量应该很大吧？设计实验以及编写文档的助教和大家的想法是一样的！所以肯定不会让大家从零开始实现一个词法分析器。在 `task1` 中我们提供了 `flex` 和 `antlr` 两种框架来实现我们的词法分析器，其中 `antlr` 在 `task2` 中还会继续用到。同学们可以自由选择自己喜欢的框架进行实现，或者让我们提供的大模型服务提供选择建议。
...
1. 预处理源代码
   ...
   点击左侧 `Cmake`扩展，在 `PROJECT OUTLINE/YatCC/test/task0` 里选择构建`task0-answer`。该操作将自动将所有`YatCC/test/cases/*/*.sysu.c`源代码进行预处理。预处理结果将保存在`YatCC/build/test/task0`文件夹中，作为词法分析器的输入。
...
!!! note "调试应循序渐进"
   ...
   调试顺序建议是：先确保 token 别名正确，再完成 Loc 的识别，最后确定是否输出 `[StartOfLine]` 和 `[LeadingSpace]`。
   这样可以确保功能逐步完善，不至于逻辑混乱。

3. 手动编写测例

   同学们在通过 task1-score 的所有测例之后，如果想检验词法分析器的功能是否完备、健壮，可以在 test/diy-cases 这个目录下自己编写测例。

!!! note "注意，测例文件必须以 .sysu.c为后缀，否则无法被识别。"
...
4. 打包提交
   ...
   完成实验后，将实验报告上传到 `/YatCC/task/task1` ，然后构建 `PROJECT OUTLINE/YatCC/task/task1-score` 进行实验一源代码打包，打包结果将保存于 `/YatCC/build/task`中。请将打包好的源代码提交给助教，我们会在后台进行隐藏测例的评测并汇总分数。
...
1. 本地评测标准
   ...
   注：`eof`这个 token 的位置和源文件路径不计入评分。`eof` 需要被识别，但它的位置和源文件路径不计分。
...
***最后 task1 的评测成绩：本地评测分数 * 0.95 + 隐藏测例分数 * 0.05***
...
- `/YatCC/test/task1/CMakeLists.txt`：主要包含两个构造目标：
  ...
  - `task1-score`：调用同文件夹下的 `score.py`，将 `task1` 生成的输出与 `clang` 生成的标准答案进行比较，最终统计各测例得分。评分时会根据测例权重文件对各测例得分进行加权计算总得分。
    同时，本文件还包含为每个测例创建测试的代码，方便同学们使用断点调试功能（相关用法已在[如何调试代码](../introduction/howtouse.md#如何调试代码)介绍，此处不再赘述）。

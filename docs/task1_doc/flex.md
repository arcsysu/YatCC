...
由于枚举值并不是我们要在最终文件中输出的字符串，所以`lex.cpp`中还定义了一个字符串数组`kTokenNames`，用于保存每个枚举值对应的输出字符串。注意`kTokenNames`中的字符串顺序需要与`lex.hpp`中`enum Id`的枚举变量的顺序：例如`COMMA`在`enum Id`中位于`IDENTIFIER`开始的第14位（从0开始计算），那么`"comma"`在`kTokenNames`的下标应该是14。
...
- 补充词法规则：首先在`lex.l`中编写词法分析规则，然后给位于`lex.hpp`中的`enum Id`中添加对应的枚举值，并且在`kTokenNames`的**正确位置**添加对应的输出字符串（位置的对应关系请看[lex.cpp文件](#lex.cpp文件)）。

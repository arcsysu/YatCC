...
- YatCC-AI在线Codespace（或本地VSCode）
- 正常网络连接
- 获得助教提供的配置示例以及连接地址url

#### 配置 Roo Code
0. 在 Codespace 左侧的插件市场安装 **Roo Code**  
    ![插件安装](../images/roo_plugin.pic.jpg)
...
- 在电脑浏览器打开github，点击右上角头像，选择`Settings`
  - ![主页](../images/llmtools/1.%20gh主页.png){height=80%, width=80%}
  - ![工具栏](../images/llmtools/2.%20工具栏.png){height=600}
- 选择`Emails`，点击`Add email address`
  - ![邮箱](../images/llmtools/3.%20设置-邮箱.png){height=80%, width=80%}
- 验证邮箱，点击`Send verification email`
- 完成验证后，点击`Primary`，设置为主邮箱
  - ![验证](../images/llmtools/4.%20邮箱认证.png){height=80%, width=80%}
...
- 添加passkey
  - ![passkey](../images/llmtools/5.%20添加passkey.png){height=80%, width=80%}
- 启用2fa，点击右上角头像，选择`Settings`，选择`Security`，启用2fa。这里推荐在手机上装一个`Authenticator`和一个github mobile，然后按指示操作
  - ![2fa](../images/llmtools/6.%20添加2fa.png){height=80%, width=80%}
...
- 如果没有的话，打开[github education](https://education.github.com/discount_requests/application)
  - ![申请界面](../images/llmtools/8.%20申请界面.png){height=80%, width=80%}
- **注意：从这里开始，确保已经关闭了vpn进行操作，可以用校园网**
- role选择`Student`，对照下面的"you must:"和上述增加成功率的方法来做好准备
  - ![准备](../images/llmtools/9.%20准备.png){height=60%, width=60%}
...
- 保证这时浏览器的定位在学校范围内，并且与刚才账单信息填写的地址一致，点击Continue验证位置，进入Upload proof界面说明通过了
  - ![申请界面2](../images/llmtools/8.%20申请界面2.png){height=80%, width=80%}
- **注意：从这里开始可以重新打开vpn**
...
- 一般来说，申请提交后会在1-2天内收到邮件（在界面上红色就是被驳回，紫色是申请通过），如果被拒绝，会有一个拒绝的理由，根据理由进行修改再次提交
  - ![已通过邮件](../images/llmtools/11.%20已通过邮件.png){height=80%, width=80%}
  - ![被拒绝邮件](../images/llmtools/10.%20驳回邮件.png){height=80%, width=80%}

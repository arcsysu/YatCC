...
3. 我们已经为大家创建好了一个容器，初始为停止状态

   ![YatCC-OL使用流程](../images/environment/YatCC-OL_3.png)

   若登录后服务容器列表为空或无法启动，请点击侧边栏模板库重新创建一个工作空间，服务容器名字自选。
   ![YatCC-OL使用流程](../images/environment/re-create-container.png)

   创建好容器之后，点击侧边栏服务容器即可看到新创建的容器。

4. 选中容器点击启动（同步状态可能有延迟，手动点击一下同步即可看到容器状态为运行中）。如果出现未响应或错误提示，请等待若干分钟后重试

5. 启动后点击左侧下拉菜单可看到容器入口，点击图片中的 URL，进入工作空间
...
7. 在代码空间中按下`Ctrl+~`键，唤出终端，开始配置环境
   1. 克隆实验仓库
      ```bash
      cd /YatCC     # 切换到工作目录
      git clone https://hk.gh-proxy.org/https://github.com/arcsysu/YatCC.git . --depth 1 --branch main     # 克隆实验仓库
      ```
   2. 配置 Cmake 工作链

      克隆成功后，在左侧文件中找到 `CmakeLists.txt`：  

      ![Cmake配置](../images/environment/cmake.png)  

      配置Toolchain，选择最底下的`gcc x86` 选项  
   ...
   3. 配置Roo Code
      参考[AI Agent](https://docs.yatcc-ai.com/#/introduction/agent)小节中关于 Roo Code 的配置方法，选择在线平台提供的体验URL和体验Key进行配置。

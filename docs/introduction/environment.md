...
1. 在浏览器中进入`https://yatcc-ai.com`网站，点击网站左上角的 Get Started!图标

   ![YatCC-OL使用流程](../images/environment/banner.png)

2. 输入学生学号与密码（默认为`hElLo! `+`学号`），点击登录键。注意是英文感叹号，感叹号和学号之间有空格。

   ![YatCC-OL使用流程](../images/environment/login.png)

3. 我们已经为大家创建好了一个名为“yatcc-2026”的容器，初始为停止状态

   ![YatCC-OL使用流程](../images/environment/init-container.png)

   若登录后服务容器列表为空或无法启动，请联系助教。

4. 复制访问密钥，选中容器点击启动（同步状态有延迟，等待几秒后手动点击刷新按钮即可看到容器状态为运行中）。

   ![YatCC-OL使用流程](../images/environment/launch.png)

!!! note "如果出现未响应或错误提示，请等待若干分钟后重试"
   
   启动成功后，点击绿色的 Web 平台访问入口，粘贴刚刚复制的访问密钥，进入平台

   ![YatCC-OL使用流程](../images/environment/code-server-login.png)

5. 成功进入云端代码空间，接下来开始实验环境配置

   ![YatCC-OL使用流程](../images/environment/workspace.png)

   1. 实验仓库
      我们已经为大家预先拉取了实验仓库，若大家可以看到实验仓库，请继续[配置 Cmake 工具链](#cmake-config)

      若进入容器后发现 /YatCC 目录为空，请按 `Ctrl+~` 键唤出终端，手动执行以下命令
      ```bash
      cd /YatCC     # 切换到工作目录
   ...
   2. 配置 Cmake 工具链<span id="cmake-config"></span>
      ...
      在左侧目录中找到 `CmakeLists.txt`：  

      ![Cmake配置](../images/environment/cmake.png)  

      配置Toolchain，选择最底下的`GCC 13.3.0 x86_64-linux-gnu` 选项  

      ![Cmake配置](../images/environment/toolchain.png)  
...
此时在 VSCode 的顶部居中位置会出现如下图所示的内容，请同学们在出现的搜索栏中输入 `Dev Containers` 关键词，然后大家需要点击下图红色三角形所示的按钮 `Dev Containers: Reopen in Container`。该操作表示将当前文件夹（YatCC 代码仓库）以容器的形式打开并自动进行容器环境配置。
...
待同学们观察到上图所示的进度条达到 100%或者消失，并且出现如下图所示的界面。使用 Dev Containers 进行自动构建的环境的操作就完成了。
...
从 WSL 打开 YatCC 仓库后，后续操作与[从 Windows 启动 Dev Containers](#start-dev-container-from-windows)相同。
...
2. 在搭建 Dev Containers 时若出现网络问题，请检查本机（包括 WSL2）代理是否开启以及 git 代理是否配置，并尝试在打开/关闭代理后重新搭建。
...
2. 在 Docker Desktop 中为你的容器增加 Memory limit/Swap，并重新编译。如果你使用的是 Windows+wsl，方法可能会有所不同。详情可参阅 docker 内的提示。
...
成功打开 vscode 后将如下所示
![vsc菜单](../images/environment/codemenu.png)

### 使用包管理器下载安装必备软件

在 vscode 中打开终端，使用你的包管理器下载以下软件，在 Linux 上的包管理器通常是 `apt` ，这里我使用的是 MacOS 上常用的 `brew`
...
在上述软件成功安装之后，请大家输入以下命令进行另外两个特殊软件的安装,两种软件在对应文件夹下都有助教提前写好的自动化编译安装脚本。但由于 MacOS 不支持 `lld` 链接器，直接使用 cmake 编译安装 llvm 源文件会报错，这里推荐直接使用官网提供的二进制包。
...
当 CMake Tools 插件安装完成后，点击删除缓存并重新配置，当看到 “xx done" 字样时，你就成功完成配置了 🎉
...
1. 在 MacOS 下，cmake 找不到 brew 安装的 bison
   ...
   解决方法：将 `task/CMakeLists.txt`中第 2 行的`find_package(BISON 3.8)`改为`find_package(BISON)`，然后重新配置 cmake。

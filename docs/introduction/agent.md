...
3. **选择模型**：官方建议不要在 Token 上过度吝啬——Roo 的多步代理能力极度依赖高质量模型的推理逻辑。推荐使用 `claude-3-5-sonnet`（擅长代码理解与指令遵循）或 `gpt-4o`（擅长复杂生成）。
 ...
 > **第一节课时**：由于还未正式发放 API Key，我们将使用一个体验URL和体验Key，如下：
 > Base URL: `http://725466.proxy.nscc-gz.cn:8888/v1`
 > API Key: `sk-HgydNOthjGB5kxbGCf1aF81a77C447F5B00d9111AaDe4dBd`
 > Model: `deepseek-chat`/`kimi-k2.5`/`minimax-m2.5`/`deepseek-reasoner`（任选其一，建议使用deepseek-chat于简单任务task0）
...
![上下文引用@](../images/llmtools/上下文引用@.gif)
...
![auto approve](../images/llmtools/auto%20approve.gif)
...
![自动检查](../images/llmtools/自动检查点与回滚.gif)
---
...
![预制斜杠命令](../images/llmtools/预制斜杠命令.gif)

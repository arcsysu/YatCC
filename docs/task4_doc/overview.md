...
<div class="quote-card">
  <p class="quote-card__eyebrow">Task 4 · 代码优化 / Code Optimization</p>
  <p class="quote-card__text">&quot;We should forget about small efficiencies, say about 97% of the time: premature optimization is the root of all evil.&quot;</p>
  <p class="quote-card__translation">（我们应该在97%的时间里忘掉微小的效率提升：过早优化是万恶之源。）</p>
  <p class="quote-card__author">Donald Knuth</p>
</div>

!!! warning "先守住正确性，再追求性能"

    这一页最容易犯的错误，是在优化还没站稳时就过早追逐分数。请先确保输出行为与标准程序一致，再讨论性能提升与策略设计。
...
在本次实验中，需要实现一个 LLVM IR 优化器，包含两个任务，均需要同学们去完成：

- **传统编译优化**：编写 LLVM Pass，实现经典编译优化算法
- **基于 Agent 的编译优化方法**：使用 Agent 来优化代码

传统编译优化实验的输入与输出均为 LLVM
...
本实验包含传统编译优化与基于 Agent 的编译优化方法，两部分测例独立，前者可在
`YatCC/test/cases/performance/` 目录下查看，后者则在
`YatCC/test/cases/llm-performance`
目录下提供了部分参考测例，可以自行增加测例，后续将会详细描述，传统编译优化部分将会执行测例计算加速比并评分，而基于 Agent 的编译优化方法需要根据大家的实验设计来进行评估，最后两部分加权求和作为实验四的总分。

对于传统编译优化部分，评分考虑两个方面——**正确性**与**程序运行性能**，比较对象为 clang 开启 O2 优化后生成的程序（下称**标准程序**）以及：
...
- 程序运行性能：通过正确性验证后，进行性能测试。对于单个实验测例，其得分为标准程序的运行时间与同学们优化后的程序运行时间求比值后开平方，再乘以 100（单个测例最高分为 100 分）。某部分实验的得分，为这部分实验所有测例的得分的均值。

上面得到的性能总分并非该实验的最终分数，同学们不需要追求性能满分。我们还将综合考虑同学们实现的优化算法进行赋分，测例也可能会修改，**以最终公示的评分规则为准**。

对于基于 Agent 的编译优化方法，这部分实验要求是灵活的，评分将根据设计的 Agent 优化目标来选择这个标准，将会包括**消耗 token 数**和**正确性**，以及可能会有**程序运行性能**、**代码体积**和**分析召回率**等。同学们需要自己选择测试集以及相应的测试指标来说明自己 Agent 的设计的有效性。在
[代码框架介绍](../task4_doc/framework.md)
的大语言模型方法章节中给出了一个简单的示例。

即在基于 Agent 的编译优化方法实验中，同学们需要实现一个 Agent，这个 Agent 的目标可以由同学们自己选择，我们在
[基于 Agent 的优化方法](../task4_doc/llm4compiler.md)
中给出了一些参考方向，实现语言可以使用 C/C++或者 Python，实验方法不限，可以基于现有方法框架上进行改造，但是需要在实验报告中体现。最后提交内容需要包括：代码、测试集（可以自己编写也可以下载现成的）和实验报告，实验报告中，需要给出对实现 Agent 的介绍、选择的评价指标、使用测试集的介绍，以及实验结果和复现指令，即同学们需要在实验报告中体现出自己实现的 Agent 是能够有效的处理选择的问题的。
...
!!! tip "先看是否正确，再看是否变快"
    ...
    优化题最常见的误判，是一看到时间数据就开始调性能，却没有先确认语义是否已经偏离。
    输出、返回值和日志先对齐，再分析性能变化，排错路径会干净很多。
...
!!! warning "只在必要时重跑全量 score"

    全量评分适合收尾验收，不适合每改一处就跑一遍。
    开发过程中先用单测例或小范围样例缩短反馈周期，最后再做全量检查。

...
本次实验的基于 Agent 的编译优化方法框架允许大家使用 C/C++或者 Python 语言，并且推荐大家使用 Pyhton 语言，目前常见的小型 Agent 基本都是基于 Python 语言实现的，如果想要使用其他语言，需要在实验中给出详细的实验环境配置操作，方便助教进行复现。

实验框架中给出了一个示例实现，主要分为以下两部分：
...
这部分内容在框架的代码中并没有体现，因为示例实现的 PassSequencePredict 没有使用工具，但是对于一个 Agent 来说，工具调用实际上是必须的，比如我们可以简单地使用编译来编译代码，用于验证大模型生成的代码的词法和语法正确性。这也是为什么推荐使用 Python 语言来进行这部分实验，同学们可以按照自己的需求来使用相应的工具，但是需要提供详细的工具获得与安装指令，
